...
public class PETimer
{
    ...
    /// <summary>
    /// 自定义处理任务的逻辑；
    /// 1.如果这个委托为空，驱动函数在执行任务时，当任务回调满足条件（时间已到）会自动调用回调；
    /// 2.如果这个委托不为空，驱动函数在执行任务时，当任务回调满足条件（时间已到），将任务回调和任务id作为参数传递给该委托，并执行该委托；
    /// 使用Timer的好处是：Timer会自动使用线程池中的线程进行驱动，驱动检测不需要放置在主线程中调用从而增加主程的负担
    /// 即通过执行taskHandle来代替执行callBack本身
    /// </summary>
    ...
    /// <summary>
    /// PETimer的构造器；
    /// 如果不指定参数或参数为0，则需要在构造实例后使用循环或者其它驱动函数内调用Update函数驱动；
    /// 如果指定一个参数，将使用System.Timers.Timer对Update进行（异步）驱动；
    /// 需要注意的是，无论是否使用Timer，最终的计时都需要使用Update驱动
    /// </summary>
